var org_metaworks_website_EssenceFile = function(objectId, className){
...
org_metaworks_website_EssenceFile.prototype.extraFilename = function(filepath){
...
org_metaworks_website_EssenceFile.prototype.setFilename = function(filename){
...
org_metaworks_website_EssenceFile.prototype.addFileChangeListener = function(listener){
	this.fileChangeListeners[this.fileChangeListeners.length] = listener;
};

org_metaworks_website_EssenceFile.prototype.showStatus = function(message){
	if('upload DONE.' == message){
		$("#objDiv_" + this.objectId).triggerHandler('uploaded');
	}
};

org_metaworks_website_EssenceFile.prototype.reset = function(){
...
org_metaworks_website_EssenceFile.prototype.add = function(event){
	$(mw3.getInputElement(this.objectId, 'fileTransfer')).trigger(event);
}


org_metaworks_website_EssenceFile.prototype.del = function(event){
	var object = mw3.objects[this.objectId];
...
org_metaworks_website_EssenceFile.prototype.download = function(){		
	mw3.call(this.objectId, 'download');
}
org_metaworks_website_EssenceFile.prototype.getImage = function(){		
	mw3.call(this.objectId, 'download');
}
