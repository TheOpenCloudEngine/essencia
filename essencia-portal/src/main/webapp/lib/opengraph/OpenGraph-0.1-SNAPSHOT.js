...
OG.renderer.IRenderer.prototype = {
    ...
    /*
	_createTerminal: function(element, line){
        var terminal, distance, intersectPoints;
        intersectPoints = element.shape.geom.intersectToLine(line);
...
OG.renderer.RaphaelRenderer.prototype._drawGeometry = function (groupElement, geometry, style, parentStyle) {
	...
	case OG.Constants.GEOM_TYPE.COLLECTION:
		for (i = 0; i < geometry.geometries.length; i++) {
			// recursive call
			geometry.geometries[i].style.map = geometry.style.map;
...
OG.renderer.RaphaelRenderer.prototype.drawShape = function (position, shape, size, style, id) {
	...
	/*
	if(groupNode.geom.boundary._upperLeft.x < 0){
		me.move(groupNode, [Math.abs(groupNode.geom.boundary._upperLeft.x), 0]);
	}
	if(groupNode.geom.boundary._upperLeft.y < 0){
		me.move(groupNode, [0, Math.abs(groupNode.geom.boundary._upperLeft.y)]);
	}
	*/
...
OG.renderer.RaphaelRenderer.prototype.redrawShape = function (element, excludeEdgeId, inclusion) {
	var me = this, envelope, center, width, height, upperLeft, root, eleArray = [],
...
OG.renderer.RaphaelRenderer.prototype.redrawEdge = function (edgeElement) {
	var me = this, edge, fromTerminalId, toTerminalId, fromShape, toShape, fromElement, toElement, fromParent, toParent, fromTerminalNum, toTerminalNum,
    ...
    /*
    //refactory : 연결된 도형에 따라 선의 모양을 바꾸기 위해서
	if(me._CONFIG.ENABLE_CONTEXTMENU){
		// redraw edge
		
		$(me.getRootElement()).find("[_shape=GROUP]").each(function (index, element) {
			if(element.shape instanceof OG.shape.HorizontalPoolShape){
				var i, elements = me.getElementsByBBox(element.shape.geom.getBoundary());
				
				for(i=0;i<elements.length;i++){
					if(elements[i] == fromElement)
						fromParent = element;
		...
		if(fromShape && toShape){
			if(fromParent != toParent){
				...
				edge = this.drawEdge(new OG.Line(fromXY, toXY, pointOfInflection),
					OG.Util.apply(edge.shape.geom.style.map, {"edge-direction": fromDrct + " " + toDrct, "stroke-dasharray": "--", "arrow-start":"open_oval-wide-long", "arrow-end": "open_block-wide-long"}), edge.id, isSelf, pointOfInflection);
			}else if(fromShape.attributes._shape_id.value == "OG.shape.bpmn.Value_Chain" 
						|| toShape.attributes._shape_id.value == "OG.shape.bpmn.Value_Chain"){
				edge = this.drawEdge(new OG.Line(fromXY, toXY, pointOfInflection),
					OG.Util.apply(edge.shape.geom.style.map, {"edge-direction": fromDrct + " " + toDrct, "stroke-dasharray": "", "arrow-end": "none"}), edge.id, isSelf, pointOfInflection);
			}else if(fromShape.attributes._shape_id.value == "OG.shape.bpmn.Value_Chain_Module" 
						|| toShape.attributes._shape_id.value == "OG.shape.bpmn.Value_Chain_Module"){
				edge = this.drawEdge(new OG.Line(fromXY, toXY, pointOfInflection),
					OG.Util.apply(edge.shape.geom.style.map, {"edge-direction": fromDrct + " " + toDrct, "stroke-dasharray": "", "arrow-end": "none"}), edge.id, isSelf, pointOfInflection);
			}else{
				edge = this.drawEdge(new OG.Line(fromXY, toXY, pointOfInflection),
					OG.Util.apply(edge.shape.geom.style.map, {"edge-direction": fromDrct + " " + toDrct, "stroke-dasharray": "", "arrow-start": "none", "arrow-end": "classic-wide-long"}), edge.id, isSelf, pointOfInflection);
			}
		}else{
			edge = this.drawEdge(new OG.Line(fromXY, toXY, pointOfInflection),
					OG.Util.apply(edge.shape.geom.style.map, {"edge-direction": fromDrct + " " + toDrct, "stroke-dasharray": "", "arrow-start": "none", "arrow-end": "classic-wide-long"}), edge.id, isSelf, pointOfInflection);		
 ...
 * @override
 */
OG.renderer.RaphaelRenderer.prototype.connect = function (from, to, edge, style, label, withTerminalId) {
	var me = this, _style = {}, fromShape, toShape, fromElement, toElement, fromParent, toParent, intersectionInfo, fromXY, toXY,
	...
	OG.Util.apply(_style, style ? style.map : style || {}, me._CONFIG.DEFAULT_STYLE.EDGE);
	...
	if (fromShape && toShape) {
		...
		if(me._CONFIG.ENABLE_CONTEXTMENU){
			...
			if(fromParent != toParent){
				$(edge).attr("connect","POOL");
				_style["arrow-start"] = "open_oval-wide-long";
...
OG.handler.EventHandler.prototype = {
	...
	enableConnect: function (element) {
		...
		$(me._RENDERER._CANVAS._CONTAINER).bind({
								...
								$(item).draggable({
									...
									stop : function (event) {
										...
										if (!$(root).data("edge_terminal") && me._isConnectCloneable(element.shape)) {
											...
											clonedElement = me._RENDERER.drawShape([me._grid(to.x, 'move'), me._grid(to.y, 'move')], element.shape.clone(),
		...
		if (isMovable === true) {
			...
			$(element).draggable({
				...
				stop : function (event) {
					...
					//이동 시 연결 선을 찾아서 연결 선을 엘리먼트 보다 아래 위치하게 만들기 위해서
					edgeId = $(element).attr("_fromedge");
					if (edgeId) {
						$.each(edgeId.split(","), function (idx, item) {
							eleArray.push($('#' + item)[0]);
						});
					}
					edgeId = $(element).attr("_toedge");
					if (edgeId) {
			...
			'fillColor'  : {
				...
				items: {
					'fillColor_select': {
						name    : '선택',
						type    : 'select',
						options : {'': '', 'white': '하양', 'gray': '회색', 'blue': '파랑', 'red': '빨강', 'yellow': '노랑', 'orange': '오렌지', 'green': '녹색', 'black': '검정'},
						selected: '',	
						events  : {
							change: function (e) {
								if (e.target.value !== '') {
...
//			this._CONFIG.MOVABLE = false;
//			this._CONFIG.RESIZABLE = false;
//			this._CONFIG.CONNECTABLE = false;
//			this._CONFIG.SELF_CONNECTABLE =false;
//			this._CONFIG.CONNECT_CLONEABLE = false;
//			this._CONFIG.CONNECT_REQUIRED = false;
//			this._CONFIG.LABEL_EDITABLE = false;
//			this._CONFIG.GROUP_DROPABLE = false;
//			this._CONFIG.GROUP_COLLAPSIBLE = false;
//			this._CONFIG.ENABLE_HOTKEY = false;
//			this._CONFIG.ENABLE_CONTEXTMENU = false;
		}
		...
		if (json && json.opengraph && json.opengraph.cell && OG.Util.isArray(json.opengraph.cell)) {
			...
			for (i = 0; i < cell.length; i++) {
				...
				case OG.Constants.SHAPE_TYPE.EDGE:
					...
					element = this.drawShape(null, shape, null, OG.JSON.decode(style), id, null, false);
					break;
				case OG.Constants.SHAPE_TYPE.HTML:
					shape = eval('new ' + shapeId + '()');
					if (value) {
					...
					element = this.drawShape([x, y], shape, [width, height, angle], OG.JSON.decode(style), id, null, false);
					break;
				case OG.Constants.SHAPE_TYPE.IMAGE:
					shape = eval('new ' + shapeId + '(\'' + value + '\')');
					if (label) {
